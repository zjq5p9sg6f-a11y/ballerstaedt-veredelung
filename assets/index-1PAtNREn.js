 ...
 */
class RGBELoader extends ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.DataTextureLoader {
	...
	constructor( manager ) {
		 ...
		 */
		this.type = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.HalfFloatType;
	...
	parse( buffer ) {
		...
		const RGBEByteToRGBHalf = function ( sourceArray, sourceOffset, destArray, destOffset ) {
			...
			destArray[ destOffset + 0 ] = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.DataUtils.toHalfFloat( Math.min( sourceArray[ sourceOffset + 0 ] * scale, 65504 ) );
			destArray[ destOffset + 1 ] = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.DataUtils.toHalfFloat( Math.min( sourceArray[ sourceOffset + 1 ] * scale, 65504 ) );
			destArray[ destOffset + 2 ] = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.DataUtils.toHalfFloat( Math.min( sourceArray[ sourceOffset + 2 ] * scale, 65504 ) );
			destArray[ destOffset + 3 ] = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.DataUtils.toHalfFloat( 1 );
		...
		switch ( this.type ) {
			...
			case ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.FloatType:
				...
				data = floatArray;
				type = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.FloatType;
				break;

			case ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.HalfFloatType:
				...
				data = halfArray;
				type = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.HalfFloatType;
				break;
		...
		function onLoadCallback( texture, texData ) {
			...
			switch ( texture.type ) {
				...
				case ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.HalfFloatType:
					...
					texture.colorSpace = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.LinearSRGBColorSpace;
					texture.minFilter = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.LinearFilter;
					texture.magFilter = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.LinearFilter;
...
const SMAAEdgesShader = {
	...
	uniforms: {
		...
		'tDiffuse': { value: null },
		'resolution': { value: new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Vector2( 1 / 1024, 1 / 512 ) }
...
class SMAAPass extends Pass {
	...
	constructor( ) {
		...
		this._edgesRT = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.WebGLRenderTarget( 1, 1, {
			depthBuffer: false,
			type: ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.HalfFloatType
		} );
		this._edgesRT.texture.name = 'SMAAPass.edges';

		this._weightsRT = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.WebGLRenderTarget( 1, 1, {
			depthBuffer: false,
			type: ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.HalfFloatType
		} );
		...
		this._areaTexture = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Texture();
		this._areaTexture.name = 'SMAAPass.area';
		this._areaTexture.image = areaTextureImage;
		this._areaTexture.minFilter = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.LinearFilter;
		...
		this._searchTexture = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Texture();
		this._searchTexture.name = 'SMAAPass.search';
		this._searchTexture.image = searchTextureImage;
		this._searchTexture.magFilter = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.NearestFilter;
		this._searchTexture.minFilter = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.NearestFilter;
		this._searchTexture.generateMipmaps = false;
		this._searchTexture.flipY = false;

		// materials - pass 1

		this._uniformsEdges = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.UniformsUtils.clone( SMAAEdgesShader.uniforms );

		this._materialEdges = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.ShaderMaterial( {
		...
		this._uniformsWeights = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.UniformsUtils.clone( SMAAWeightsShader.uniforms );
		...
		this._materialWeights = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.ShaderMaterial( {
		...
		this._uniformsBlend = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.UniformsUtils.clone( SMAABlendShader.uniforms );
		this._uniformsBlend[ 'tDiffuse' ].value = this._weightsRT.texture;

		this._materialBlend = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.ShaderMaterial( {
...
const stepGeoCache = /* @__PURE__ */ new Map();
...
function alignShortestAxisToY(geometries) {
  ...
  if (size.y <= size.x && size.y <= size.z) {
    ...
    rotation = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Matrix4().makeRotationX(-Math.PI / 2);
  } else {
    rotation = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Matrix4().makeRotationZ(-Math.PI / 2);
  }
  if (rotation) {
    for (const g of geometries) {
...
function generatePlanarUV(geometries) {
  const bbox = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Box3();
  ...
  for (const g of geometries) {
    ...
    g.setAttribute("uv", new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.BufferAttribute(uvs, 2));
  }
}
async function loadStep(url) {
  ...
  const p = (async () => {
    ...
    for (const m of result.meshes) {
      ...
      if (m.index) {
        g.setIndex(new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.BufferAttribute(Uint32Array.from(m.index.array), 1));
      }
    ...
    for (const g of geometries) bbox.union(g.boundingBox);
    const size = bbox.getSize(new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Vector3());
    const cadDiameterMm = Math.max(size.x, size.z);
...
preloadAllSteps();
function buildStepGroup(stepGeo, material, diameterMm, mmToScene, flipY) {
  const stepMaterial = material.clone ? material.clone() : material;
  if (stepMaterial && stepMaterial.side !== undefined) {
    stepMaterial.side = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.FrontSide;
  }
  const inner = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Group();
  for (const g of stepGeo.geometries) {
    const mesh = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Mesh(g, stepMaterial);
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    inner.add(mesh);
  }
  const center = stepGeo.bbox.getCenter(new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Vector3());
...
new RGBELoader().setPath("./textures/").load(
  "studio.hdr",
  (texture) => {
    texture.mapping = ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.EquirectangularReflectionMapping;
...
function rebuildFoil() {
  ...
  if (stepUrl) {
    const myGroup = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Group();
    foilGroup = myGroup;
...
const TIEFGEZOGEN_FORMS = /* @__PURE__ */ new Set(["verformt-lasche", "verformte-ronde"]);
const KUNSTSTOFF_ALLOWED_PRAEGUNGEN = /* @__PURE__ */ new Set(["glatt", "nadelstich"]);
function isKunststoff(matId) {
  return matId === "kunst";
}
function isShapeAllowed(shapeId, matId) {
  if (isKunststoff(matId) && TIEFGEZOGEN_FORMS.has(shapeId)) return false;
  return true;
}
function isPraegungAllowed(praegungId, matId) {
  if (isKunststoff(matId) && !KUNSTSTOFF_ALLOWED_PRAEGUNGEN.has(praegungId)) return false;
  return true;
}
function applyDisabledStyle(el, disabled, reason) {
...
function setShapeProgrammatic(newShapeId) {
...
function setPraegungProgrammatic(newPraegungId) {
...
resize();
const clock = new ballerstaedt_mf_2_veredelung__loadShare__three__loadShare__.Clock();
function animate() {
